/**
 * Monthly Comparison — Compare two monthly reports, compute deltas and insights.
 *
...
function avgVoteReasonRate(report: MonthlyReportData): number | null {
...
function avgEvidenceCount(report: MonthlyReportData): number | null {
...
export function compareMonthlyReports(
  ...
  // Class-level deltas (only for classes active in either month)
  ...
  const classDeltas: ClassDelta[] = Array.from(classMap.entries())
  ...
  const scenarioDeltas: ScenarioDelta[] = Array.from(scenarioMap.entries())
  ...
  // Insights
  const insights = computeComparisonInsights(deltas, classDeltas);
...
function computeComparisonInsights(
  deltas: SummaryDeltas,
  classDeltas: ClassDelta[],
): Insight[] {
  ...
  if (accDelta != null) {
    if (accDelta > DELTA_THRESHOLD_RATE) {
      insights.push({
        type: 'observation',
        text: `正解率が前月より${formatDeltaPct(accDelta)}上昇しており、推理整理が進んでいる可能性があります`,
      });
    } else if (accDelta < -DELTA_THRESHOLD_RATE) {
      insights.push({
        type: 'observation',
        text: `正解率が前月より${formatDeltaPct(Math.abs(accDelta))}低下しており、難易度の調整が必要かもしれません`,
      });
    } else {
      insights.push({
        type: 'observation',
        text: '正解率は前月とほぼ同水準を維持しています',
      });
    }
  }
  ...
  if (disDelta != null && accDelta != null) {
    ...
    if (disDelta > DELTA_THRESHOLD_TIME) {
      insights.push({
        type: 'observation',
        text: `議論時間が前月より${formatDeltaTime(Math.abs(disDelta))}増加しています`,
      });
    } else if (disDelta < -DELTA_THRESHOLD_TIME) {
      insights.push({
        type: 'observation',
        text: `議論時間が前月より${formatDeltaTime(Math.abs(disDelta))}減少しています`,
      });
    }
  }
  ...
  if (vrDelta != null) {
    if (vrDelta > DELTA_THRESHOLD_RATE) {
      insights.push({
        type: 'observation',
        text: `投票理由の記入率が前月より${formatDeltaPct(vrDelta)}上がっており、根拠の言語化が定着しつつあります`,
      });
  ...
  if (sesDelta != null) {
    if (sesDelta > 0) {
      insights.push({
        type: 'observation',
        text: `授業回数が前月より${sesDelta}回増え、実施経験が蓄積されています`,
      });
    } else if (sesDelta < 0) {
      insights.push({
        type: 'observation',
        text: `授業回数が前月より${Math.abs(sesDelta)}回減少しています`,
      });
    }
  }
  ...
  for (const cd of classDeltas.slice(0, 3)) {
    if (cd.accuracyDelta != null && Math.abs(cd.accuracyDelta) > DELTA_THRESHOLD_RATE) {
      ...
      insights.push({
        type: 'observation',
        text: `${cd.className}の正解率が前月より${formatDeltaPct(Math.abs(cd.accuracyDelta))}${dir}しています`,
      });
    }
  }
  ...
  if (insights.length === 0) {
    insights.push({
      type: 'observation',
      text: '前月と大きな変化は見られず、安定した授業運営が続いています',
    });
  }
...
/** Format a delta value for display. Returns "+X" / "-X" / "±0" string */
export function formatDeltaDisplay(
  d: DeltaValue,
  unit: 'pct' | 'time' | 'count' | 'pctPt',
): { text: string; color: 'up' | 'down' | 'neutral' } {
  if (d.delta == null) return { text: '--', color: 'neutral' };

  const sign = d.delta > 0 ? '+' : d.delta < 0 ? '' : '±';
  const direction: 'up' | 'down' | 'neutral' =
    d.delta > 0 ? 'up' : d.delta < 0 ? 'down' : 'neutral';

  switch (unit) {
    case 'pct':
    case 'pctPt': {
      const pts = Math.round(d.delta * 100);
      return { text: `${sign}${pts}pt`, color: direction };
    }
    case 'time': {
      const absSec = Math.abs(Math.round(d.delta));
      const m = Math.floor(absSec / 60);
      const s = absSec % 60;
      const timeStr = m > 0 ? (s > 0 ? `${m}:${String(s).padStart(2, '0')}` : `${m}:00`) : `0:${String(s).padStart(2, '0')}`;
      return { text: `${sign}${d.delta < 0 ? '-' : ''}${timeStr}`.replace('--', '-'), color: direction };
    }
    case 'count': {
      const val = Math.round(d.delta);
      return { text: `${sign}${val}`, color: direction };
    }
  }
}

/** Get appropriate color classes for delta direction */
export function deltaColorClass(
  ...
  // For "positive" metrics (higher is generally better): up=green, down=red
  if (metric === 'positive') {
    return direction === 'up' ? 'text-green-600' : 'text-red-500';
  }

  // For "negative" metrics (lower is generally better): up=red, down=green
  if (metric === 'negative') {
    return direction === 'up' ? 'text-red-500' : 'text-green-600';
  }

  // Neutral metrics: just show direction without judgment
