...
import { supabase, checkAndAwardBadges, fetchStudentStreak, fetchStudentAssignments } from '../../lib/supabase';
      ...
      <div ref={contentRef} class="flex-1 overflow-y-auto px-4 py-4">
        <div class="max-w-lg mx-auto">
          ...
          {step === STEP_TRUTH && (
            <SoloTruthStep
              rpEarned={rpEarned}
              revealedCount={revealedSecrets.size}
              witnessCount={witnessCount}
              readEvidenceCount={readEvidence.size}
              totalEvidenceCount={data.evidenceCards.length + (data.evidence5 ? 1 : 0)}
              vote={vote}
              voteReason={voteReason}
              solutionHtml={data.solutionHtml}
              challengeHtml={data.challengeHtml}
              showConfetti={showConfetti}
              newBadges={newBadges}
              streakInfo={streakInfo}
              feedbackData={feedbackData}
              nextScenario={nextScenario || null}
              currentSeriesName={data.seriesName}
              isPerspective={isPerspective}
              hypothesisSuspect={hypothesisSuspect}
